...
def parse_args():
    ...
    parser.add_argument(
        "--model",
        choices=["rf", "xgb","svr","all"],
        default="all",
        help="Model: 'rf' (RandomForest), 'xgb' (XGBoost), 'svr' (SupportVectorRegression),'all' (tots).",
    )
    parser.add_argument(
        "--kfold",
        type=int,
    ...
    try:
        ...
        print("   python projecte_preus.py --model svr")
        print("   python projecte_preus.py --model all --kfold 10")
        print("\nValors per defecte.")
...
def train_svr(X_train, y_train, categorical_cols, numeric_cols):
    # Log-transform del target per estabilitzar la variança
    y_train_log = np.log1p(y_train)  # log1p evita problemes amb zero

    # Pipeline de preprocessament
    preprocessor = ColumnTransformer(
        transformers=[
            ('num', StandardScaler(), numeric_cols),  # Escalem variables numèriques
            ('cat', OneHotEncoder(handle_unknown='ignore', sparse_output=False), categorical_cols)
        ]
    )
    ...
    svr_model = SVR(
        ...
        C=10,              # penalització d’error (major C = menor error en train)
        epsilon=0.05,        # marge dins del qual errors no són penalitzats
        gamma='scale'       # coeficient del kernel
    )

    pipe = Pipeline(steps=[
        ('preprocessor', preprocessor),                 # Escala i codifica categories
        ('pca', PCA(n_components=30, random_state=42)), # Redueix dimensionalitat
        ('scaler', StandardScaler()),                   # Escala PCA components
        ('model', svr_model)
    ])

    # Entrena el pipeline amb el target log-transform
...
def cross_val_svr(X, y, categorical_cols, numeric_cols, n_splits=5):
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=42)
    ...
    for i, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        # Entrena pipeline SVR amb log-transform
        pipe = train_svr(X_train, y_train, categorical_cols, numeric_cols)

        # Prediccions en escala log → tornen a escala original
...
def main():
    ...
    if args.kfold > 0:
        ...
        if args.model in ["svr", "all"]:
            print(f"\n=== K-Fold Cross Validation SVR ({args.kfold} folds) ===") 
            cross_val_svr(X, y, categorical_cols, numeric_cols, n_splits=args.kfold)
    ...
    # -------------------------------
    if args.model in ["svr", "all"]:
        print("\n\n" + "="*20 + " SVR (Support Vector Regression) " + "="*20)
        
        # 1. Entrenar (El model entrena internament amb logaritmes)
        pipe_svr = train_svr(X_train, y_train, categorical_cols, numeric_cols)
        
        # 2. Predir en test (El resultat surt en escala logarítmica)
        y_pred_log = pipe_svr.predict(X_test)
        
        # 3. Transformació inversa (Log -> Real) per avaluar correctament
        y_pred_svr = np.expm1(y_pred_log)

        # 4. Avaluar
        regresion_avaluation(y_test, y_pred_svr)

        # 5. Overfitting Manual (Calculem manualment perquè el .score() usaria log vs log)
        # Predim sobre train i desfem el logaritme per comparar preus reals
        y_train_pred_real = np.expm1(pipe_svr.predict(X_train))
        
        r2_train_real = r2_score(y_train, y_train_pred_real)
        r2_test_real = r2_score(y_test, y_pred_svr)

        print("\nAnàlisi Overfitting SVR (Escala Real)")
        print(f"R2 Train: {r2_train_real:.4f} | R2 Test: {r2_test_real:.4f}")
        
        gap_svr = r2_train_real - r2_test_real
        if gap_svr > 0.10:
            print(f"Overfitting alt (Diferencia: {gap_svr:.2%}).")
        else:
            print(f"Generalització correcta (Diferencia: {gap_svr:.2%}).")
    # terminal python projecte_preus_v5.py(arxiu última version) --model (rf,xgb,svr,all) --kfold 5 
